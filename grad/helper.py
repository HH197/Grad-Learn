...
def entropy_batch_mixing(latent_space, batches, K=50, n_jobs=8, n=100, n_iter=50):
    """
    Adopted from:
    ...
    for t in range(n_iter):
        ...
        score += np.mean([entropy(batches[inds[i]]) for i in range(n)])
...
def plot_tSNE(latent, labels, cmap=plt.get_cmap("tab10", 7), perform_tsne=True):
    """
    Adoptd from:
...
def HC(latent, labels, num_clusters=[2, 3, 4, 7]):
    """
    ...
    for i in num_clusters:
        ...
        nmi = metrics.cluster.normalized_mutual_info_score(new_labels, hc.labels_)
        nmis.append(nmi)
